...
#text(style: "italic")[__repr__(), repr_html(), repr_latex(), ...] --- внутренние методы Pandas для вывода таблицы. Когда мы вызываем методы типа ```python .head()```, ```python .tail()```, под капотом задействуются в том числе они.
...
Но хотим ли мы загружать данные целиком?
...
/ Разреженные данные: данные, где большинство признаков имеет нулевое значение.

Ещё полезные методы:
- `pd.arrow` --- Arrow-backed string 
- `pd.convert_types`
...
=== Numba
Представляет собой JIT-компилятор, переводит подмножество операций Python и NumPy в машинный код через LLVM, с помощью пакета llvmlite.

Когда мы скачиваем намбу, скачивается дополнительный пакет, который осуществляет этот переход.
...
Это где какие реализации посчитаны. Первый in --- это встроенная питоновская функция

Pandas не сильно отстал от Numba, поэтому давайте заставим его это сделать. Напишем функцию возведения в квадрат и посчитаем среднее.
